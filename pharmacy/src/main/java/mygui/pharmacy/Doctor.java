        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
            ...
            for (Order order : currentOrders) {
                ...
                for (Product product : order.getOrderedProducts()) {
                    writer.write(product.getName() + "," + product.getProductId() + "," + 
                                 product.getPrice() + "," + product.getQuantity() + "\n");
                }
                writer.write("TotalPrice: " + order.calculateTotalPrice() + "\n");
            }
          
            System.out.println("Doctor data saved to " + fileName);
        } catch (IOException e) {
            System.err.println("Error saving doctor data to file \"" + fileName + "\": " + e.getMessage());
        }
    }
    ...
    public void loadFromFile(String fileName) {
        try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
            ...
            for (int i = 0; i < orderCount; i++) {
                ...
                String totalPriceLine = reader.readLine();
                double totalPrice = Double.parseDouble(totalPriceLine.split(": ")[1]);
